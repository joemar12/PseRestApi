...
public class TestAsyncEnumerable<T, TExpressionVisitor> : TestQueryProvider<T, TExpressionVisitor>,
    IAsyncEnumerable<T>, IAsyncQueryProvider
    where TExpressionVisitor : ExpressionVisitor, new()
{
    private static readonly MethodInfo _fromResultMethod = typeof(Task).GetMethod(
        nameof(Task.FromResult), BindingFlags.Public | BindingFlags.Static)
        ?? throw new InvalidOperationException("Task.FromResult method not found.");

    private readonly Action<T> _removeCallback;

    public TestAsyncEnumerable(Expression expression, Action<T> removeCallback) : base(expression)
    {
        _removeCallback = removeCallback;
    }

    public TestAsyncEnumerable(IEnumerable<T> enumerable, Action<T> removeCallback) : base(enumerable)
    {
        _removeCallback = removeCallback;
    }
    ...
    protected override object CreateInstance(Type tElement, Expression expression)
    {
        ...
        var instance = Activator.CreateInstance(
            queryType,
            typeof(T) == tElement ? [expression, _removeCallback] : new object[] { expression, null! });

        return instance ?? throw new InvalidOperationException(
            $"Failed to create instance of {queryType.FullName}");
    }
    ...
    public TResult ExecuteAsync<TResult>(Expression expression, CancellationToken cancellationToken = default)
    {
        ...
        return (TResult)_fromResultMethod
            .MakeGenericMethod(expectedResultType)
    ...
    private static object ExtractValue(Expression expr, T item)
    {
        ...
        if (expr is ConstantExpression constExpr)
            return constExpr.Value ?? throw new InvalidOperationException("Constant expression returned null.");
        ...
        if (expr is LambdaExpression lambda)
        {
            ...
            {
                0 => del.DynamicInvoke() ?? throw new InvalidOperationException(
                    "Lambda returned null."),
                1 => del.DynamicInvoke(item) ?? throw new InvalidOperationException(
                    "Lambda returned null."),
        ...
        return lambdaNoParams.Compile().DynamicInvoke() ?? throw new InvalidOperationException("Lambda returned null.");
    }
    ...
    private static LambdaExpression ExtractLambda(Expression expr)
    {
        ...
        return (expr as LambdaExpression) ?? throw new InvalidOperationException("Expression is not a lambda expression.");
    }
